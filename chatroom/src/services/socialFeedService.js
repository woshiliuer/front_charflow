...
export const publishSocialFeed = async ({ content = '', files = [] } = {}) => {
  return apiClient.post('/socialFeed/publish', {
    content: content || '',
...
export const commentSocialFeed = async ({ feedId, content } = {}) => {
  return apiClient.post('/socialFeedComment/comment', { feedId, content })
}
...
export const uploadSocialFeedFiles = async (files = []) => {
  ...
  list.forEach((f) => { if (f) formData.append('files', f) })
